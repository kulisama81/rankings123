# Inspector Report — 2026-08-11

## Summary

Completed live QA sweep of rankings123.com. Found **1 critical (P0) core feature regression**.

## Routes Checked

- ✅ `/` (Homepage) — multi-sport sections present
- ❌ `/atp-live` — **CRITICAL: pagination missing** (P0)
- ✅ `/wta-live` — pagination works correctly (100 players, Page 1/2)
- ⚠️ `/world-cup` — "Live" status bug already filed (bug-wc-live-status-regression)
- ✅ `/privacy` — loads correctly

## Automated Checks

- ❌ **`npm run check:core-features`** — **FAILED**
  - ATP live ranking + pagination: **no pagination control** ❌
...
### 1. ATP Live: Pagination Missing, Only 40 Players (P0 — CORE FEATURE REGRESSION)

**Ticket:** `bug-atp-pagination-missing-deep-ranking`

**Severity:** P0 (Critical)

**Description:** ATP Live page shows only 40 players with NO pagination controls. Should show ~1000 players with pagination (50/page) per core features requirements.

**Evidence:**
- Live site inspection: 40 rows, no pagination UI
- Core features check: FAILS with "no pagination control"
- WTA comparison: WTA shows 100 players with working pagination

**Root cause hypothesis:** Deep ranking data (`getAtpDeepRankingData()`) not loading properly — either fetch failure or data not being passed to component.

**Impact:** 
- Breaks protected core feature
- Blocks core-features gate
- Users cannot access players ranked 41+
- Major UX regression vs competitors

**Regression test required:** Test that ATP deep ranking returns 500+ players

---

## Already-Filed Bugs Confirmed Still Present

- `bug-wc-live-status-regression` (P1) — World Cup page shows "Live" status when tournament is complete (verified: still shows "Live now" with no upcoming fixtures)

## Routes Clean

- Homepage: multi-sport sections present, navigation works
- WTA Live: pagination working correctly (Page 1/2, 100 players)
- Privacy: loads correctly, no broken links

## Browser Testing Notes

- Tested with headless Chromium via Playwright
- No JavaScript console errors observed on clean routes
- Theme toggle not tested (needs interactive session)

## Next Actions

Highest priority: **`bug-atp-pagination-missing-deep-ranking`** (P0) — this is blocking the core-features gate and breaks a protected feature. The planner should pick this up immediately.

## Test Coverage Gaps Identified

The core-features check correctly caught the ATP pagination regression. The check is working as designed. However, it would be useful to add a data-layer test that verifies `getAtpDeepRankingData()` returns a minimum player count (500+) to catch data source failures earlier.

---

**Inspector:** Automated QA sweep  
**Date:** 2026-08-11  
**Duration:** ~15 minutes  
**Tickets filed:** 1 (P0)
