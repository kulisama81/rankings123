...
export async function generateMetadata({
  ...
  const title = `Best ${filter.label} ATP Players ${month} ${year} | Live Rankings`;
  const description = `Live ATP ranking for ${filter.label.toLowerCase()} players ${month} ${year}. Real-time points, rank movement, and tournament progress for the ${filter.label.toLowerCase()} age group.`;

  return {
    title,
    description,
    alternates: { canonical: `/atp/${ageFilter}` },
    openGraph: {
...
export default async function AtpAgeRankingPage({
  params,
}: {
  params: Promise<{ ageFilter: string }>;
}) {
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: "Home", url: "/" },
    { name: "ATP Rankings", url: "/atp-live" },
  ...
  const jsonLd = {
    ...
    "@type": "WebPage",
    name: `ATP ${filter.label} Rankings — Rankings123`,
    description: `Live ATP ranking for ${filter.label.toLowerCase()} players. Real-time points, rank movement, and tournament progress.`,
    url: `https://rankings123.com/atp/${ageFilter}`,
    inLanguage: "en",
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-6">
          <h1 className="font-display text-3xl font-bold text-fg sm:text-4xl">
            Best {filter.label} ATP Players
          </h1>
          <p className="mt-2 text-muted">
            Live ranking for {filter.label.toLowerCase()} ATP players, filtered by age. Updates in real-time during tournaments.
          </p>
          <div className="mt-4 rounded-lg bg-surface border border-edge p-4">
            <p className="text-sm text-secondary">
              <strong>{rankedPlayers.length}</strong> players aged{" "}
              {filter.maxAge ? `${filter.maxAge} and under` : `${filter.minAge}+`} currently ranked in the ATP.
              Showing their overall ATP rank and live points.
            </p>
          </div>
        </div>
