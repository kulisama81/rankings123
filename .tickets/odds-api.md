...
Wire The Odds API (the-odds-api.com, free tier) for match odds / win-probabilities for tennis (ATP/WTA) + World Cup fixtures. Feeds wc-odds + tennis-odds prediction features and the betting-affiliate slots. Key via env (ODDS_API_KEY), cached/revalidated, graceful fallback. HANDOFF: Loic creates a free The Odds API account + provides the key.
